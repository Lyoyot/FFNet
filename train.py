...
from utils.data_loader import CDNet2014Loader1
...
parser = argparse.ArgumentParser(description='FFNet pyTorch')
...
transforms_tr_fpm = [
    ...
    [aug.ToTensor()],
    [aug.NormalizeTensor(mean_rgb=mean_seg, std_rgb=std_seg,
                         mean_seg=mean_seg, std_seg=std_seg, segmentation_ch=seg_ch)]
]
...
dataloader_tr_fpm = CDNet2014Loader1(
                    dataset_tr, empty_bg=empty_bg, recent_bg=recent_bg,
                    segmentation_ch=seg_ch, transforms=transforms_tr_fpm,
)
...
dataloader_test_fpm = CDNet2014Loader1(
                    dataset_test, empty_bg=empty_bg, recent_bg=recent_bg,
                    segmentation_ch=seg_ch, transforms=transforms_test_fpm,
)

tensorloader_tr = torch.utils.data.DataLoader(
    dataset=dataloader_tr, batch_size=batch_size, shuffle=True, num_workers=1
)
tensorloader_tr_fpm = torch.utils.data.DataLoader(
    dataset=dataloader_tr_fpm, batch_size=batch_size, shuffle=True, num_workers=1
)
tensorloader_test = torch.utils.data.DataLoader(
    dataset=dataloader_test, batch_size=batch_size, shuffle=False, num_workers=1
)
tensorloader_test_fpm = torch.utils.data.DataLoader(
    dataset=dataloader_test_fpm, batch_size=batch_size, shuffle=False, num_workers=1
)
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, num_epochs):  # loop over the dataset multiple times
        for phase, tensorloader in [("Train", tensorloader_tr), ("Test", tensorloader_test)]:
            ...
            if phase == "Train":
                optimizer.zero_grad()
                tensorloader_temp = tensorloader_tr_fpm
            else:
                tensorloader_temp = tensorloader_test_fpm
            #for i, data in enumerate(tensorloader):
            for (i, data), (j, data1) in zip(enumerate(tensorloader), enumerate(tensorloader_temp)):
                if phase == "Train":
                    model.train()
                else:
                    model.eval()
                ...
                if phase == "Train":
                    ...
                    inputs_fpm, labels1 = data1[0].float(), data1[1].float()
                    if cuda:
                        inputs, labels, inputs_fpm = inputs.cuda(), labels.cuda(), inputs_fpm.cuda()
                    outputs = model(inputs, inputs_fpm)
