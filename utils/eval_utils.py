...
from utils.data_loader import CDNet2014Loader, CDNet2014Loader1
...
def evalVideo(cat, vid, model, empty_bg=False, recent_bg=False, segmentation_ch=False, eps=1e-5, 
    ...
    transforms_fpm = [
        [aug.ToTensor()],
        [aug.NormalizeTensor(mean_rgb=[0.5], std_rgb=[0.5],
                             mean_seg=[0.5], std_seg=[0.5], segmentation_ch=segmentation_ch)]
    ]
    ...
    dataloader_fpm = CDNet2014Loader1({cat: [vid]}, empty_bg=empty_bg, recent_bg=recent_bg,
                                 segmentation_ch=segmentation_ch, transforms=transforms_fpm,
                                 use_selected=use_selected, multiplier=0)
    tensorloader = torch.utils.data.DataLoader(dataset=dataloader,
                                               batch_size=1,
                                               shuffle=False,
                                               num_workers=1)
    tensorloader_fpm = torch.utils.data.DataLoader(dataset=dataloader_fpm,
                                               batch_size=1,
                                               shuffle=False,
                                               num_workers=1)
    ...
    for (i, data), (j, data1) in zip(enumerate(tensorloader),enumerate(tensorloader_fpm) ):
        ...
        input_fpm, label_fpm = data1[0].float(), data1[1].float()
        input, label = input.cuda(), label.cuda()
        input_fpm, label_fpm = input_fpm.cuda(), label_fpm.cuda()
        ...
        input = zeropad(input)
        input_fpm = zeropad(input_fpm)
        output = model(input, input_fpm)
