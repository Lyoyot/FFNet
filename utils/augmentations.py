...
class NormalizeTensor:
    ...
    def __call__(self, inp, out):
        ...
        if self.segmentation_ch:
            mean_period = np.concatenate((self.mean_seg, self.mean_rgb.copy()))
            std_period = np.concatenate((self.std_seg, self.mean_rgb.copy()))
        else:
            mean_period = self.mean_rgb.copy()
            std_period = self.std_rgb.copy()
        ...
        if num_frames > 1:
            ...
            inp_n = tvtf.Normalize(mean_period, std_period)(inp)
        return inp_n, out
